	...
	context("detects a Java Native Image app", func() {
		...
		it("builds successfully", func() {
			...
			image, logs, err = pack.Build.
				...
				WithBuilder(Builder).
				WithEnv(map[string]string{"BP_NATIVE_IMAGE": "true", "BP_JVM_VERSION": "25", "BP_MAVEN_BUILD_ARGUMENTS": "-Pnative --batch-mode -Dmaven.test.skip=true --no-transfer-progress package"}).
				Execute(name, source)
