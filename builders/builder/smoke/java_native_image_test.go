...
func testJavaNativeImage(t *testing.T, context spec.G, it spec.S) {
	...
	context("detects a Java Native Image app", func() {
		...
		it("builds successfully", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "spring-boot-native-image-maven"))
			Expect(err).NotTo(HaveOccurred())
			...
			image, logs, err = pack.Build.
				...
				WithBuilder(Builder).
				WithEnv(map[string]string{"BP_NATIVE_IMAGE": "true", "BP_JVM_VERSION": "25", "BP_MAVEN_BUILD_ARGUMENTS": "-Pnative --batch-mode -Dmaven.test.skip=true --no-transfer-progress package", "USE_NATIVE_IMAGE_JAVA_PLATFORM_MODULE_SYSTEM": "false"}).
				Execute(name, source)
